...
class Issuer extends BaseIssuingStateMachine
{
    ...
    ): bool
    {
        ...
        try {
            ...
            $expires_time = date('Y-m-d H:i:s', time() + $this->time_to_live);
            ...
            if (!$resp instanceof RequestCredentialMessage) {
                throw new StateMachineTerminatedWithError(
                    self::OFFER_PROCESSING_ERROR, 'Unexpected @type: '.(string)$resp->getType()
                );
            }
            ...
            if ($ack instanceof Ack || $ack instanceof CredentialAck) {
                ...
                throw new StateMachineTerminatedWithError(
                    self::ISSUE_PROCESSING_ERROR, 'Unexpected @type: '.(string)$resp->getType()
                );
            }
